	...
	<div class="wrapper">
		<div class="model" v-show="model" @click="model = false">
			...
			<div class="container" style="width:20%;margin-top:-10%;margin-left:44%" v-show="modelBtn" @click="modelBtn = false" >
				<div style="margin-left:-25%;margin-top:-2%" ><font size="2" color="#ffffff" face="微软雅黑">✱ 【确定】后将会上传图片为【个人头像】</font><br></div>
  ...
  data: function () {
	...
	methods: {
		sumitImageFile(base64Codes){
			...
			axios.post("/api/teacher/upload",formData).then(response => {				
				this.headImgUrl = response.data.result
				this.$refs.cropper2.clearCrop()
				this.$emit('headImage',this.headImgUrl)
